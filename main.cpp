...
// 5. MOTOR DEL ALGORITMO EVOLUTIVO
Cromosoma ejecutarAlgoritmoEvolutivo(int num_dias, int num_periodos, int num_clases, int num_profesores, 
    ...
    for (int gen = 0; gen < generaciones; ++gen) {
        ...
        // Elitismo: Conservar a los 2 mejores
        ...
        while (nueva_poblacion.size() < tamano_poblacion) {
            ...
            Cromosoma padre = poblacion[rand() % (tamano_poblacion / 3)]; 
            Cromosoma hijo = padre; // Clonación inicial
            
            // Operador de Mutación: SWAP INTRA-CLASE
            int clase_mutar = rand() % num_clases;
            int d1 = rand() % num_dias, p1 = rand() % num_periodos;
            int d2 = rand() % num_dias, p2 = rand() % num_periodos;
            
            swap(hijo.matriz[d1][p1][clase_mutar], hijo.matriz[d2][p2][clase_mutar]);
            
            hijo.evaluarFitness(reqs, num_profesores, indisponibilidad);
            nueva_poblacion.push_back(hijo);
        }
        poblacion = nueva_poblacion;
        if(gen % 20 == 0) cout << "Generacion " << gen << " - Mejor Fitness (Costo Penalizaciones): " << poblacion[0].fitness << endl;
    }
    return poblacion[0];
};
